...
// Rewrite lucide-react named imports to use import* + destructuring with fallback,
// so icons that don't exist in the loaded version degrade to null instead of crashing.
function rewriteLucideImports(code: string): string {
  const lucideImportRegex = /import\s*\{([^}]+)\}\s*from\s*['"]lucide-react['"]/g;
  let hasLucideImport = false;

  const rewritten = code.replace(lucideImportRegex, (_, imports: string) => {
    hasLucideImport = true;
    const names = imports.split(',').map((n: string) => n.trim()).filter(Boolean);
    const destructured = names.map((n: string) => {
      const parts = n.split(/\s+as\s+/);
      if (parts.length === 2) {
        return `${parts[0].trim()}: ${parts[1].trim()} = _LucideFallback`;
      }
      return `${n} = _LucideFallback`;
    });
    return `import * as _lucideAll from 'lucide-react';\nconst { ${destructured.join(', ')} } = _lucideAll`;
  });

  if (hasLucideImport) {
    return `const _LucideFallback = () => null;\n` + rewritten;
  }
  return rewritten;
}
...
export function transformJSX(
  ...
  try {
    ...
    // Rewrite lucide-react named imports to gracefully handle missing icons
    processedCode = rewriteLucideImports(processedCode);
